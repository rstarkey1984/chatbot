...
- 사용자는 여러 화면/상황에서 대화를 이어감
- 서로 다른 대화 주제가 같은 세션에 섞이면 문맥 충돌 발생
- 같은 대화 주제가 서로 다른 세션으로 갈라지면 문맥 단절 발생

핵심 질문:  
**"같은 주제는 이어지고, 다른 주제는 섞이지 않게 만들 수 있는가?"**

발표 멘트(3줄):
- 사용자 입장에서는 대화가 자연스럽게 이어지는 것이 가장 중요합니다.
- 그런데 실제로는 대화가 섞이거나 끊기면서 사용 경험이 떨어졌습니다.
- 그래서 같은 주제는 이어지고 다른 주제는 분리되는 기준이 필요했습니다.
